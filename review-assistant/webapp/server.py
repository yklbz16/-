...
class SummaryRequest(BaseModel):
    subject: str = ""
    mode: str = ""  # mindmap / compare / cheatsheet / chain
    chapter: str = ""
    concept_a: str = ""
    concept_b: str = ""
    start_concept: str = ""
...
def _split_questions(text: str) -> list[dict]:
    ...
    for i, part in enumerate(parts):
        ...
        if not part:
            continue
        # 提取题号标题（**第X题**）
        title_match = re.search(r'\*\*第\s*(\d+)\s*题\*\*[（(]([^）)]*)[）)]', part)
        number = title_match.group(1) if title_match else str(i + 1)
        topic = title_match.group(2) if title_match else "综合"
        # 移除标题行剩下的就是题目正文
        body = part
        if title_match:
...
## 格式要求（重要）
每道题之间用单独一行「---」分隔。每道题开头用「**第X题**（知识点：XXX）」标注题号和所属知识点。
例如：
**第1题**（知识点：栈的定义）
请简述栈的LIFO特性及其应用场景。
...
请直接按此格式输出题目。"""
    ...
    try:
        response = await provider.chat([
            {"role": "system", "content": "你是一位经验丰富的出题老师，擅长根据知识点设计高质量的考题。请严格按格式要求输出，每题之间用 --- 分隔。"},
            {"role": "user", "content": prompt}
        ])
        # 尝试将题目文本拆分为结构化列表
        questions = _split_questions(response)
        return {"ok": True, "questions_text": response, "questions": questions, "mode": data.mode}
    except Exception as e:
